...
def ptb_producer(raw_data, batch_size, num_steps, name=None):
    with tf.name_scope(name, "PTBProducer", [raw_data, batch_size, num_steps]):
        ...
        row_len = data_len // (num_steps+1)
        data = tf.reshape(raw_data[0 : (num_steps+1) * row_len], [-1, num_steps+1])
        epoch_size = (row_len - 1) // (batch_size)
...
def ptb_producer_test(raw_data, batch_size, num_steps, name=None):
    with tf.name_scope(name, "PTBProducer", [raw_data, batch_size, num_steps]):
        ...
        with tf.control_dependencies([assertion]):
            epoch_size = tf.identity(epoch_size, name="epoch_size")
        i = tf.train.range_input_producer(epoch_size, shuffle=False).dequeue()
        x = tf.strided_slice(data, [0, i * (num_steps+1)], [batch_size, (i + 1) * (num_steps+1)])[:,:num_steps]
        x.set_shape([batch_size, num_steps])
