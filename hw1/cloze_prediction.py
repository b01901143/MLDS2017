...
print ('test data path: %s' %test_path)
#print (ans_path)
if train_flag:
	print ("regular training start")
else :
	print ("testing only")
#data
...
test_datasets, test_optionsets = get_questions(test_num_steps , test_path), get_options(test_path)
...
# if pretrained x are id, otherwise words
    if pretrained==None:
		...
		embed = tf.nn.embedding_lookup(pretrainEmbd.lookupEmbd(word_to_id),x)
    return embed
